...
const Content = (props: Props) => {
  ...
  const queryCategory = searchParams.get('prefecture')
  ...
  React.useEffect(() => {
    ...
    if (queryCategory) {
      data = props.data.filter((shop) => {
        return shop['都道府県'] === queryCategory
      })
    }
  ...
  return (
    <div id="shop-list" className="shop-list">
      {queryCategory && <div className="shop-list-category">{`都道府県：「${queryCategory}」${data.length}件`}</div>}
